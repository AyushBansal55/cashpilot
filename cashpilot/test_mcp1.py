...
async def main() -> None:
    async with stdio_client(SERVER) as (read, write):
        async with ClientSession(read, write) as session:
            ...
            # 5. Forecast (the analytical tool)
            fc = unwrap(await session.call_tool("run_forecast", {"horizon_days": 75}))
            s = fc["summary"]
            print(f"run_forecast → lowest €{s['lowest_point_balance']:,.0f} on "
                  f"{s['lowest_point_date']}; crisis={s['has_real_crisis']}; "
                  f"{s['shortfall_count']} shortfall(s)")

            print("\n✓ All five MCP tools responded correctly over the protocol.")
